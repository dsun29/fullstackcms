...
export default class StepZilla extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      nextStepText: props.nextStepText ? props.nextStepText : 'Next'
    };
  ...
  _getNavStates(indx, length) {
    ...
    for (let i=0; i<length; i++) {
      if (i < indx || (indx == (length -1) && i == indx)) {
        styles.push('done');
      }
      else if (i === indx) {
        styles.push('doing');
  ...
  _checkNavState(currentStep){
    if (currentStep > 0 && currentStep !== this.props.steps.length - 1) {
      let correctNextText =  this.props.steps[currentStep].nextStepText ? this.props.steps[currentStep].nextStepText : 'Next';
